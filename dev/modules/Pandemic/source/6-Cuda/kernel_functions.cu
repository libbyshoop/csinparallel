...
/*
    cuda_susceptible()
        Spawns threads to handle susceptible personales
*/
...
*/
__global__ void cuda_updateDays(char *states_dev, int *num_days_infected_dev,
    char INFECTED)
{
    // set up thread id
    int id = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    struct const_t *constant, struct stats_t *stats, struct cuda_t *cuda)
{
    ...
    cuda_updateDays<<<cuda->numBlock, cuda->numThread>>>(
