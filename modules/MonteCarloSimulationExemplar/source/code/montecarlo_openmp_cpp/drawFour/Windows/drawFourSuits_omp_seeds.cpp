...
unsigned int seeds[nThreads];   // holds seeds for each thread

// In this version we will set different seeds for each thread
void seedThreads() {
    int my_thread_id;
    unsigned int seed;
...
//	int mid = rand_rIntBetween(low, hi);	//get split point in mid fifth of deck
...
//				if (rand_r()%2 > 0) {	//take card from low half
