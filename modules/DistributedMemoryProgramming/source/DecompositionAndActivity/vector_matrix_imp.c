...
int main(int argc, char *argv[] ) {
    ...
    /* Initialize Matrix and Vector */
        ...
        numworkers = nprocs - 1;        /* number of workers in the communicator MPI_COMM_WORLD */
    ...
    if (rank > 0) {
        ...
        MPI_Recv(&matrix, rows*WIDTH, MPI_INT, 0, mtype, MPI_COMM_WORLD, &status);
        MPI_Recv(&vector, WIDTH, MPI_INT, 0, mtype, MPI_COMM_WORLD, &status);
        ...
        MPI_Send(&result, rows, MPI_INT, 0, mtype, MPI_COMM_WORLD);
    }
